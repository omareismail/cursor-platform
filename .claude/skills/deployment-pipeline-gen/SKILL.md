---
name: deployment-pipeline-gen
description: "deployment-pipeline-gen generates the CI/CD workflows that make the platform's guard rules hold outside the editor — build, test, security scan, the lifecycle gate check, container build, and a staged deploy. It is the counterpart to /devops-audit, which reviews pipelines that already exist. Invoked as /deployment-pipeline-gen."
---

# deployment-pipeline-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/deployment-pipeline-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/deployment-pipeline-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** deployment-pipeline-gen - [one-line description].`
