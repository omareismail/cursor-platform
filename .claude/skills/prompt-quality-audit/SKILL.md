---
name: prompt-quality-audit
description: "Scores a user prompt or spec for ambiguity, missing acceptance criteria, unstated constraints and hidden assumptions, then returns a rewritten version plus the clarifying questions worth asking. Use when a request is vague, or when asked to review or improve a prompt or spec before implementation. Invoked as /prompt-quality-audit."
---

# prompt-quality-audit

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/prompt-quality-audit/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/prompt-quality-audit/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** B (read-only analysis) - announce, then proceed
   - Announcement (unless Category D): `**Matched skill:** prompt-quality-audit - [one-line description].`
