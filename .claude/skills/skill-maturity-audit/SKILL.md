---
name: skill-maturity-audit
description: "Audits the skill library itself against the shared execution pipeline contract - checks each .cursor/skills/*/skill.md for correct dependency wiring, pattern-finder usage, memory references, output paths and category. Use when asked to verify skills are wired correctly, or after adding or editing a skill. Invoked as /skill-maturity-audit."
---

# skill-maturity-audit

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/skill-maturity-audit/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/skill-maturity-audit/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** B (read-only analysis) - announce, then proceed
   - Announcement (unless Category D): `**Matched skill:** skill-maturity-audit - [one-line description].`
