...
# dotnet-background-job-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/dotnet-background-job-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/dotnet-background-job-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** dotnet-background-job-gen - [one-line description].`
