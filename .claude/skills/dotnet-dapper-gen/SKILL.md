---
name: dotnet-dapper-gen
description: "dotnet-dapper-gen generates parameterized Dapper queries and repository methods for read-heavy or reporting paths where EF Core's tracking and change-detection overhead isn't worth paying — this is the deliberate alternative named in technologyStack.md, not a fallback for when EF Core 'feels slow.' Every query is parameterized regardless of provider (no string interpolation, ever —... Invoked as /dotnet-dapper-gen."
---

# dotnet-dapper-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/dotnet-dapper-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/dotnet-dapper-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** dotnet-dapper-gen - [one-line description].`
