---
name: dotnet-observability-gen
description: "dotnet-observability-gen wires structured logging, distributed tracing, and metrics into a .NET handler, endpoint, or whole API project. It matches whatever observability stack is already declared in memory-bank/techContext.md (Serilog, OpenTelemetry, Application Insights) rather than introducing a new one. Invoked as /dotnet-observability-gen."
---

# dotnet-observability-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/dotnet-observability-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/dotnet-observability-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** dotnet-observability-gen - [one-line description].`
