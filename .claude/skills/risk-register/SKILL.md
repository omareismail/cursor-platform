---
name: risk-register
description: "risk-register produces the phase 2 risk register: what could make this product fail, how likely, how bad, who owns it, and — the part that makes it useful — which phase is responsible for retiring each risk. It is deliberately broader than /threat-model, which runs in phase 3 and covers only what an attacker could do. Invoked as /risk-register."
---

# risk-register

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/risk-register/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/risk-register/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** risk-register - [one-line description].`
