---
name: react-hook-gen
description: "react-hook-gen generates a complete, typed custom React hook with a corresponding test stub. Every hook returns a named object (never an array), uses QUERY_KEYS constants (never inline string arrays), and has all return values explicitly typed. Invoked as /react-hook-gen."
---

# react-hook-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/react-hook-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/react-hook-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** react-hook-gen - [one-line description].`
