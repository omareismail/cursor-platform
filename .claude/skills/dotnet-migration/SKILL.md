...
description: "dotnet-migration generates an EF Core migration safely by first validating that the DbContext model matches the spec's Database Changes section, then reviewing the generated migration for unintended destructive changes, missing indexes, and large-table risks before any database update command is run. For any migration strategy that is not straightforward (large production tables, multi-step... Invoked as /dotnet-migration."
---

# dotnet-migration

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/dotnet-migration/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/dotnet-migration/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** dotnet-migration - [one-line description].`
