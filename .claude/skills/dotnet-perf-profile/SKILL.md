---
name: dotnet-perf-profile
description: "dotnet-perf-profile performs a static review of a handler or endpoint for common .NET performance pitfalls, and generates a BenchmarkDotNet scaffold for the hot path. It explicitly distinguishes between what belongs in a micro-benchmark (allocations, serialization cost, in-process logic) and what needs a load test (throughput under concurrency, database behaviour under parallel load). Invoked as /dotnet-perf-profile."
---

# dotnet-perf-profile

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/dotnet-perf-profile/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/dotnet-perf-profile/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** C (docs/diagrams) - announce, then proceed
   - Announcement (unless Category D): `**Matched skill:** dotnet-perf-profile - [one-line description].`
