...
**Last updated:** 2026-09-09 (review-fix pass R1–R14)
...
| D — Orchestration | E-20, E-21 | **Not started** |
...
| ID | What to do | Why |
|---|---|---|
| **E-20** | `.cursor/skills.index.json` — category, phase, capability, workflow, requires. Lint it from `sync-skills` / `docs-lint`. SessionStart prints capability rows. Fix catalog drift | Routing today is prose in `skill-catalog.md`. An index the tools can read is what stops a new skill shipping without a category or a phase |
| **E-21** | Docs-lint patterns: hyphenated names, `.mdc` links, Arabic (`HANDBOOK.ar.md`) | Counts and links still drift; Arabic handbook was never in the same lint as the English one |
