...
```bash
node .cursor/tools/project.mjs init [--name N] [--existing]
node .cursor/tools/project.mjs status|scan|snapshot|map|check [--json]
node .cursor/tools/project.mjs scan [--max-files N] [--max-depth N] [--write]
node .cursor/tools/project.mjs dashboard

node .cursor/tools/project.mjs delivery list|show|start|complete|objective|catalog|phase
node .cursor/tools/project.mjs checkpoint list|show|verify|pass|fail|waive|set
