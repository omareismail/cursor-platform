...
```bash
node .cursor/tools/omniroute.mjs tiers     # env only, no network — the one the skills call
node .cursor/tools/omniroute.mjs status    # adds one loopback GET /v1/models
node .cursor/tools/omniroute.mjs models    # the catalog, grouped by provider
```
...
```bash
node .cursor/tools/omniroute.mjs status
```
...
**Governance profiles.** `node .cursor/tools/lifecycle.mjs product` derives the
...
(`.cursor/rules/03-react-architecture-guard.mdc`); the equivalent registration is
`axios.create({ baseURL, timeout })`.

**Gateway-side fallback does not count.** `.cursor/tools/failure-modes.mjs` reads
...
`node .cursor/tools/failure-modes.mjs explain LlmGatewayClient` to see what is
...
```bash
node .cursor/tools/omniroute.mjs status          # 0 pinned · 1 a blocking finding · 2 no gateway
node .cursor/tools/omniroute.mjs tiers --json    # what each tier resolves to, and which are protected
node .cursor/tools/omniroute.mjs models --filter claude
node .cursor/tools/lifecycle.mjs product         # integrations, derived from .mcp.json
