...
If a human has run Graphify on the repo, `.cursor/tools/graphify.mjs` reads its
...
`.cursor/tools/omniroute.mjs` says which real model sits behind each Claude tier:
...
completion. The rules and the hardening checklist: `.cursor/docs/OMNIROUTE.md`.
...
`node .cursor/tools/context-cost.mjs report` asks what it costs — the always-on
