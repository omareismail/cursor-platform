...
```bash
node .cursor/tools/artifact-schema.mjs ready <STORY-ID>
```
...
```bash
node .cursor/tools/task-graph.mjs graph    specs/plans/<slug>-plan.md   # what must land first
node .cursor/tools/task-graph.mjs validate specs/plans/<slug>-plan.md   # no task over 8 files
```
