...
```bash
node .cursor/tools/artifact-schema.mjs graph
```
...
```bash
node .cursor/tools/change-request.mjs impact BR-4 FR-2
```
...
```bash
node .cursor/tools/change-request.mjs open \
...
```bash
node .cursor/tools/change-request.mjs close CR-0001 --note "<what shipped>"
```
