...
`.cursor/rules/` (all guards), `templates/dotnet/BannedSymbols.txt`
...
| 6 | Guard rule in `.cursor/rules/` | At generation time — advisory |
...
node .cursor/tools/feature-map.mjs query --file <file involved>
node .cursor/tools/delivery-metrics.mjs report --days 30
```
...
```bash
node .cursor/tools/incidents.mjs open \
