...
becomes demoable and where integration risk sits. `.cursor/rules/05-planning-rigor.mdc`
...
```bash
node .cursor/tools/task-graph.mjs validate specs/plans/[feature-slug]-plan.md
node .cursor/tools/task-graph.mjs graph    specs/plans/[feature-slug]-plan.md
```
...
```bash
node .cursor/tools/task-graph.mjs split specs/plans/[feature-slug]-plan.md TASK-004
```
