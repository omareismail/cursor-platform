...
the map and grep. Run `node .cursor/tools/graphify.mjs status`, then
`node .cursor/tools/graphify.mjs neighbours <file-or-symbol> --depth 2` for each
...
```bash
node .cursor/tools/feature-map.mjs query --file  <changed-file>
node .cursor/tools/feature-map.mjs query --table <changed-table>
