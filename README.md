...
An agent platform for **.NET + React** development in GCC fintech contexts (RHODES, Tamkeen, and similar codebases). It provides 99 skills, 12 guard rules, 14 subagents, 10 enforcement hooks, and a two-tier memory-bank so the agent discovers real repo structure, imitates existing patterns, and enforces architecture, security, audit trails, and RTL/i18n discipline.
...
.cursor/
  ...
  skills/            # 99 slash-command skills — SINGLE SOURCE OF TRUTH
...
project/             # Command Center overlay: identity, delivery, ideas
                     #   written only by project.mjs; dashboard is a projection

.claude/
  skills/            # 99 generated shims → .cursor/skills/ (never edit by hand)
  skills/_descriptions.json  # hand-written description overrides
  agents/            # 14 specialist subagents (context isolation; read-only by convention)
  hooks/             # 10 enforcement hooks + sync-skills.mjs (plain Node, no deps)
...
| [PROJECT-COMMAND-CENTER.md](.cursor/docs/PROJECT-COMMAND-CENTER.md) | Delivery phases, checkpoints, ideas, readiness — overlay on lifecycle |
...
| [skill-catalog.md](.cursor/docs/skill-catalog.md) | Full 99-skill catalog and auto-routing |
...
| [HARDENING-STATUS.md](.cursor/docs/HARDENING-STATUS.md) | Governance hardening: implemented vs verified vs attested vs committed |
| [LIFECYCLE.md](.cursor/docs/LIFECYCLE.md) | Six phases, gates, inheritance, integrity, evidence chain |
...
- **99 skills** — generators (`dotnet-endpoint-gen`, `react-component-gen`, …), auditors (`database-audit`, `compliance-audit`, …), **feature analysis** (`feature-trace`, `impact-analysis`, `feature-inventory`, `spec-drift-audit`), **work breakdown** (`work-breakdown` + speckit), **verification** (`task-verify`), **outer loop** (`production-readiness-review`, `operability-gen`, `release-safety`, `threat-model`, `delivery-metrics`, `postmortem`), **project command center** (`project`, `dashboard`)
...
No license file is selected yet. That is an owner decision (H12), not something
this repository infers. `.claude-plugin/plugin.json` says `SEE LICENSE IN README.md`
because that is where the decision will be recorded. Add a `LICENSE` file if you
intend to distribute the platform beyond the team.
