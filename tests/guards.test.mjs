...
console.log("\nguard-bash.mjs — the refusals, and the exemptions that make them usable");
{
  ...
  const cases = [
    ...
    ["pip install requests",               true,  "a new Python package"],
    ["python -m pip install -U requests",   true,  "a Python package behind python -m pip"],
    ["uv add --dev pytest",                true,  "a uv dev dependency behind --dev"],
    ["pip install -r requirements.txt",    false, "a declared requirements restore"],
    ["pip install -e .",                   false, "an editable install of the repo itself"],
    ["uv sync",                            false, "a uv lockfile restore"],
    ["uvx ruff check .",                   true,  "an ephemeral uvx run"],
    ["uv run ruff check .",                false, "a tool from the project's own environment"],
    ["npx skills add owner/repo",          true,  "a skill fetched from a remote repository"],
    ["npx skills check",                   false, "a skills command that installs nothing"],
