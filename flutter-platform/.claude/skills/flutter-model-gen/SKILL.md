...
description: "Generates an immutable freezed + json_serializable data class with JSON (de)serialization, correct @JsonKey mapping for API field names, and a round-trip test. Use when asked to add a model, entity, or DTO. Invoked as /flutter-model-gen."
---

# flutter-model-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/flutter-model-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/flutter-model-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** flutter-model-gen - [one-line description].`
