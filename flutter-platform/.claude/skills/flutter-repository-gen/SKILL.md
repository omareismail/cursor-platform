...
description: "Generates the data layer for a resource: an abstract repository interface, its API-client-backed implementation with typed error mapping, and the provider binding, so the application layer only depends on the interface. Use when asked to add a repository, API client, or data source. Invoked as /flutter-repository-gen."
---

# flutter-repository-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/flutter-repository-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/flutter-repository-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** flutter-repository-gen - [one-line description].`
