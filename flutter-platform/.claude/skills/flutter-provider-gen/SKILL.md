---
name: flutter-provider-gen
description: "Scaffolds a Riverpod Notifier or AsyncNotifier (codegen @riverpod style or manual, matching the repo's existing convention) that owns state and orchestrates repository calls via AsyncValue.guard. Use when asked to add state management, a provider, or a notifier. Invoked as /flutter-provider-gen."
---

# flutter-provider-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/flutter-provider-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/flutter-provider-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** flutter-provider-gen - [one-line description].`
