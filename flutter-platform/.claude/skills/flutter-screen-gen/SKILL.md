...
description: "Generates a full Flutter screen in one pass: the screen widget, its backing Riverpod AsyncNotifier/Notifier, and the go_router route registration, handling loading/error/empty/data states explicitly. Use when asked to build a new screen or page. Invoked as /flutter-screen-gen."
---

# flutter-screen-gen

> **Registration shim.** Canonical instructions live in
> `.cursor/skills/flutter-screen-gen/skill.md` - single source of truth, shared with Cursor.
...
1. **Read `.cursor/skills/flutter-screen-gen/skill.md` now** and follow every step literally.
2. Apply the platform contract in `AGENTS.md`:
   - **Category:** A (generates/modifies files) - announce, then wait for go-ahead
   - Announcement (unless Category D): `**Matched skill:** flutter-screen-gen - [one-line description].`
