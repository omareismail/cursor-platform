...
- `${CLAUDE_PLUGIN_ROOT}/rules/04-security-guard.mdc` (primary)
- `${CLAUDE_PLUGIN_ROOT}/rules/06-database-provider-guard.mdc` (parameterization, dialects)
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs record-gate DESIGN --verdict GO|NO-GO \\
