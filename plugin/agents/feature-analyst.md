...
|---|---|
| "How does X work?" | `/feature-trace` |
| "What breaks if I change X?" | `/impact-analysis` |
| "What does this system do?" | `/feature-inventory` |
| "Does the code match the spec?" | `/spec-drift-audit` |
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/feature-map.mjs upsert /tmp/<id>.json
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/feature-map.mjs list          # what is already known
node ${CLAUDE_PLUGIN_ROOT}/tools/feature-map.mjs verify <id>   # is it still true
```
