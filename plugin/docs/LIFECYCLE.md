...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs status              # where are we
...
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs rollback PHASE --reason "..."
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/artifact-schema.mjs check   # the full picture
node ${CLAUDE_PLUGIN_ROOT}/tools/artifact-schema.mjs trace S-07
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs override DESIGN \
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs init --name "<product>" --existing --by "<claimant>" --review-by "<second person>"
```
...
- [`${CLAUDE_PLUGIN_ROOT}/rules/11-lifecycle-gate.mdc`](../rules/11-lifecycle-gate.mdc) — the binding rule
