...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs init --name "motors-online"
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs check REQUIREMENTS
```
```
/lifecycle-gate REQUIREMENTS
```
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs record-gate REQUIREMENTS --verdict GO \\
  --by "business-analyst" --criteria "<n>/<total>"
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs approve REQUIREMENTS --by "Ezzdeen"
...
```
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs record-gate DESIGN --verdict GO \\
  --by "security-auditor" --criteria "<n>/<total>"
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs approve DESIGN --by "Ezzdeen"
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs advance
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs record-gate PRODUCTION --verdict GO \\
  --by "security-auditor" --criteria "<n>/<total>"
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs approve PRODUCTION --by "Ezzdeen"
```
...
| Next release starts | `node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs rollback DEVELOPMENT --reason "release 2"` |
