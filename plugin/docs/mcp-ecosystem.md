...
| ECC memory MCP (`ecc memory` / `memory-mcp`) | A second, competing store beside `memory-bank/` — the same objection as `server-memory`, which is why that one is two rows above. Its `ecc.memory.v1` documents are per-machine and unreviewed by design (`trust: "unreviewed"` on write), so promoting anything from them would mean trusting a record nobody read. The one idea worth having — a handoff between Cursor and Claude Code — was taken instead as `session-end.mjs`, writing to gitignored `.cursor/cache/sessions/` that is never committed and never cited as evidence. Assessed in [ecc-assessment-2026-09-16.md](../../docs/reviews/ecc-assessment-2026-09-16.md). |
| ECC plugin (`ecc@ecc`, 24 hooks) | Not an MCP server, listed here because it arrives the same way and costs the same window. Installing it adds 24 hooks that run on every tool call and are covered by no hash in `lifecycle/integrity.json`, plus 292 skill description lines to a catalogue whose existing 99 already account for half the always-on cost on Claude Code. Five mechanisms it had and this platform did not were re-implemented natively instead. Same assessment. |
