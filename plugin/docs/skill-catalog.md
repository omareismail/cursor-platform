...
below — every skill in `${CLAUDE_PLUGIN_ROOT}/skills/` has a row.
...
`${CLAUDE_PLUGIN_ROOT}/tools/ac-trace.mjs`, reads the tests for the failure modes a tool cannot
...
`${CLAUDE_PLUGIN_ROOT}/tools/task-graph.mjs` validates: max 8 files and 2 layers per task, a
...
`node ${CLAUDE_PLUGIN_ROOT}/tools/harness-scan.mjs scan` asks the third question: what the
