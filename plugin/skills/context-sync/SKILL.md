---
name: context-sync
description: "context-sync bootstraps or refreshes the entire memory-bank/ from the actual current state of the codebase. It is the entry point for using this .cursor/ setup on an existing project that has no memory-bank, and the recovery tool when memory-bank has drifted significantly out of date. Invoked as /context-sync."
---

<!-- GENERATED from the cursor-platform source skill "context-sync".
