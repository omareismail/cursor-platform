---
name: lifecycle
description: "lifecycle is the entry point to the six-phase product lifecycle: it reports which phase a product is in, which gates a human has approved, and what the agent is therefore allowed to do next. It is the product-level counterpart to the feature-level pipeline in 01-specify-rules.mdc — that one gates a single feature from analyze to commit, this one gates the whole product from an idea to production. Invoked as /lifecycle."
---

<!-- GENERATED from the cursor-platform source skill "lifecycle".
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/omniroute.mjs tiers
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs check <PHASE>     # do the artifacts exist
```
...
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs advance
```
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs override <PHASE> \
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/lifecycle.mjs rollback <PHASE> --reason "<why>"
```
