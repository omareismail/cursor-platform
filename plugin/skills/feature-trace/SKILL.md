...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/feature-map.mjs init      # no-op if it already exists
...
you spend them. Run `node ${CLAUDE_PLUGIN_ROOT}/tools/graphify.mjs status`. On exit 0 the graph
still matches the code: run
`node ${CLAUDE_PLUGIN_ROOT}/tools/graphify.mjs neighbours <entry file> --depth 2` and take its
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/tools/feature-map.mjs upsert /tmp/<feature-id>.json
node ${CLAUDE_PLUGIN_ROOT}/tools/feature-map.mjs verify <feature-id>
```
...
- **Cache:** `.cursor/cache/feature-map.json` updated via
  `node ${CLAUDE_PLUGIN_ROOT}/tools/feature-map.mjs upsert` (never hand-edited — the
