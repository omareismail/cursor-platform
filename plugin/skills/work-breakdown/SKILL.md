...
<!-- GENERATED from the cursor-platform source skill "work-breakdown".
...
2. **Task size is enforced, not estimated.** S/M/L points say how long work takes.
   ...
   by `${CLAUDE_PLUGIN_ROOT}/tools/task-graph.mjs`, which rejects that outright.
...
`${CLAUDE_PLUGIN_ROOT}/rules/05-planning-rigor.mdc` requires options with tradeoffs before any
...
node ${CLAUDE_PLUGIN_ROOT}/tools/task-graph.mjs graph    specs/plans/<slug>-tasks.md
```
